const urlQuery = {
  ...
  queryOne: function (key, req) {
    if (req) {
      ...
      var s = str ? '?' + this._getQueryFromUrl(str) : window.location.search
  ...
  queryRouterLastOne: function (req) {
    if (req) {
  ...
  search: function () {
    return window.location.hash ? window.location.hash : window.location.search
  }
}
